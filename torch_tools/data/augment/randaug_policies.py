...
Copied from:
https://github.com/google-research/uda/tree/d4ec238c5c358e9552b18a69a7091112db7db7c7/image/randaugment
"""
import random
...
def apply_policy(policy, img, use_mean_std=True):
    """Apply the `policy` to the torch `img`.
  ...
  """
    # img_shape = img.shape
    img_shape = np.array(img.shape)[[1, 2, 0]]
    mean_std = NotImplemented if use_mean_std else None
    # pil_img = pil_wrap(img, use_mean_std)
    pil_img = pil_wrap2(img, mean_std)
    for xform in policy:
        assert len(xform) == 3
    ...
    return pil_unwrap2(pil_img, mean_std)
...
def randaug_policies():
    ...
    op_list = [(trans, 0.5, magnitude) for trans in trans_list for magnitude in range(1, 10)]
    policies = [[op_1, op_2] for op_1 in op_list for op_2 in op_list]
    return policies
